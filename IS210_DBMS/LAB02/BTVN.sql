...
CREATE USER C##user02 IDENTIFIED BY password;
CREATE USER C##user03 IDENTIFIED BY password;
CREATE USER C##user04 IDENTIFIED BY password;
CREATE USER C##user05 IDENTIFIED BY password;
CREATE USER C##user06 IDENTIFIED BY password;
CREATE USER C##user07 IDENTIFIED BY password;
CREATE USER C##user08 IDENTIFIED BY password;
CREATE USER C##user09 IDENTIFIED BY password;
CREATE USER C##user10 IDENTIFIED BY password;
CREATE USER C##user11 IDENTIFIED BY password;
CREATE USER C##user12 IDENTIFIED BY password;
CREATE USER C##user13 IDENTIFIED BY password;
CREATE USER C##user14 IDENTIFIED BY password;
CREATE USER C##user15 IDENTIFIED BY password;
CREATE USER C##user16 IDENTIFIED BY password;
CREATE USER C##user17 IDENTIFIED BY password;
CREATE USER C##user18 IDENTIFIED BY password;
CREATE USER C##user19 IDENTIFIED BY password;
CREATE USER C##user20 IDENTIFIED BY password;
--CREATE ROLES
CREATE ROLE C##Role_QUANTRI;
GRANT ALL PRIVILEGES TO C##Role_QUANTRI;
CREATE ROLE C##Role_NGUOIDUNG;
GRANT SELECT ANY TABLE TO C##Role_NGUOIDUNG;
--GAN ROLE QUAN TRI
GRANT C##Role_QUANTRI TO C##user01, C##user02, C##user03, C##user04, C##user05, 
C##user06, C##user07, C##user08, C##user09, C##user10;
--GAN ROLE NGUOIDUNG
GRANT C##Role_NGUOIDUNG TO C##user11, C##user12, C##user13, C##user14, C##user15, 
C##user16, C##user17, C##user18, C##user19, C##user20;
--THU HOI/REVOKE ROLE:
REVOKE C##Role_QUANTRI FROM C##user01, C##user03, C##user05;
-----
-----
-----
-----THUCHANH.WEEK03
--BAI04: CURSOR - INFO CAC  LOP THUOC CAC MON HOC
DECLARE
    -- variables de luu tru thong tin ve mon hoc va lop hoc
    v_ma_mon COURSE.CourseNo%TYPE; --v_ma_mon type = type of CourseNo in table COURSE
    v_ten_mon COURSE.Description%TYPE;--v_ten_mon type = Description in table COURSE
    v_ma_lop CLASS.ClassID%TYPE;--v_ma_lop type = ClassID in table CLASS
    v_so_sinh_vien NUMBER;
    -- Khai bao con tro de truy xuat thong tin ve mon hoc
    CURSOR cur_mon_hoc IS
        SELECT CourseNo, Description --TRA VE CourseNo + Description
        FROM COURSE;
    -- Khai bao con tro long nhau de lay thong tin ve cac lop hoc cho moi mon hoc
    CURSOR cur_lop_hoc (p_ma_mon COURSE.CourseNo%TYPE) IS
        SELECT c.ClassID, COUNT(*) AS so_sinh_vien --Tra ve ClassID + so luong SV
        FROM CLASS c
        JOIN ENROLLMENT e ON c.ClassID = e.ClassID
        WHERE c.CourseNo = p_ma_mon
        GROUP BY c.ClassID;
BEGIN
    -- Mo con tro ben ngoai de lay thong tin ve mon hoc
    OPEN cur_mon_hoc;
    LOOP
        -- Lay thong tin ve mon hoc 
        FETCH cur_mon_hoc INTO v_ma_mon, v_ten_mon;
        EXIT WHEN cur_mon_hoc%NOTFOUND;
        -- In thong tin ve mon hoc
        DBMS_OUTPUT.PUT_LINE(v_ma_mon || ' ' || v_ten_mon || ':');
        -- Mo con tro long de lay thong tin ve cac lop hoc cho mon hoc hien tai
        OPEN cur_lop_hoc(v_ma_mon);
        LOOP
            -- Lay thong tin ve lop hoc
            FETCH cur_lop_hoc INTO v_ma_lop, v_so_sinh_vien;
            EXIT WHEN cur_lop_hoc%NOTFOUND;
            -- In thong tin ve lop hoc
            DBMS_OUTPUT.PUT_LINE('Lop: ' || v_ma_lop || ' co so luong sinh vien dang ki: ' || v_so_sinh_vien);
        END LOOP;
        -- Dong con tro long
        CLOSE cur_lop_hoc;
    END LOOP;
    -- Dong con tro ben ngoai
    CLOSE cur_mon_hoc;
END;
//
--
...
AS
    ...
    exception 
        WHEN NO_DATA_FOUND THEN 
            DBMS_OUTPUT.PUT_LINE('Khong tim thay sinh vien voi ma so ' || i_Student_id);
    END;
//
--
--
...
BEGIN
    --ultilize find_sname procedure
    find_sname(i_student_id, v_first_name, v_last_name);
    --print out names
    IF v_first_name IS NULL AND v_last_name IS NULL THEN
        RAISE_APPLICATION_ERROR(-20001, 'Khong tim thay sinh vien voi ma so ' || i_student_id);
    ELSE
    DBMS_OUTPUT.PUT_LINE('Ho va ten cua sinh vien voi ma so ' || i_student_id || ' la: ' || v_first_name || ' ' || v_last_name);
    END IF;
END;
/
--VD:
DECLARE
    v_student_id STUDENT.StudentID%TYPE := '103'; 
BEGIN
...
--
--
--5.2: discount voi mon hoc >15 sinh vien dang ky
CREATE OR REPLACE PROCEDURE Discount AS
    CURSOR cur_courses IS
        ...
        HAVING COUNT(DISTINCT e.StudentID) > 15;
    v_discount_course_no COURSE.CourseNo%TYPE;
    v_discount_course_desc COURSE.Description%TYPE;
BEGIN
    FOR course_rec IN cur_courses LOOP
        v_discount_course_no := course_rec.CourseNo;
        v_discount_course_desc := course_rec.Description;
        -- Áp d?ng gi?m giá 5% cho môn h?c có s? l??ng sinh viên ??ng kí nhi?u h?n 15
        -- Th?c hi?n các hành ??ng c?n thi?t ?? áp d?ng gi?m giá ? ?ây
        DBMS_OUTPUT.PUT_LINE('Mon hoc ' || v_discount_course_desc || ' duoc giam gia 5%');
    END LOOP;
END Discount;
/
...
--
--
--5.3: total_cost_for_student: hoc phi tong cong cua hoc sinh:
...
--vd:
DECLARE
    v_student_id STUDENT.StudentID%TYPE := '104'; 
    v_total_cost NUMBER;
BEGIN
    v_total_cost := Total_cost_for_student(v_student_id);
    IF v_total_cost IS NULL THEN
        DBMS_OUTPUT.PUT_LINE('Khong ton tai sinh vien voi ma so ' || v_student_id);
    ELSE
        DBMS_OUTPUT.PUT_LINE('Tong chi phi cho sinh vien voi ma so ' || v_student_id || ' la: ' || v_total_cost);
    END IF;
END;
/
--
--
--bai06: triggers
--6.1: trigger audit insert/update
--THAY OI EM GPT MA KHONG TIM DUOC METHODS NAO DE LAM 1 TRIGGER CHO TOAN BO CAC BANG TRONG DATABASE 
--EM DUPLICATE TRIGGERS RA TUNG TABLE NHA THAY
CREATE OR REPLACE TRIGGER add_audit_info_class
...
END;
/
CREATE OR REPLACE TRIGGER add_audit_info_course
...
END;
/
CREATE OR REPLACE TRIGGER add_audit_info_Instructor
BEFORE INSERT OR UPDATE ON INSTRUCTOR
FOR EACH ROW
BEGIN
    IF INSERTING THEN
        :NEW.CreatedBy := USER; -- NEW ROW CREATED BY 'USER'
        :NEW.CreatedDate := SYSDATE; --NEW ROW CREATED AT 'SYSDATE'
    END IF;
    IF UPDATING THEN
        :NEW.ModifyBy := USER;
        :NEW.ModifiedDate := SYSDATE; 
...
END;
/
CREATE OR REPLACE TRIGGER add_audit_info_enrollment
...
END;
/
CREATE OR REPLACE TRIGGER add_audit_info_grade
...
END;
/
CREATE OR REPLACE TRIGGER add_audit_info_student
...
--
--
--6.2: gioi han so luong mon dang ky cua hoc sinh
CREATE OR REPLACE TRIGGER GioiHanDangKyHP
...
BEGIN
    ...
    IF v_total_courses >= 4 THEN
        RAISE_APPLICATION_ERROR(-20001, 'Sinh Vien khong duoc dang ky qua 4 mon hoc.');
    END IF;
END;
/
