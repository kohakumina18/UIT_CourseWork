...
--Cauu01 
SELECT name as TenKH, id AS MaKH
FROM  s_customer
order by id DESC ;
--Cau02
--cau03
...
--CAU09 
SELECT FIRST_NAME ||' '||LAST_NAME 
AS EMPLOYEE_NAME, SALARY AS MONTHLY_SALARY
FROM S_EMP 
WHERE SALARY > 1350 AND DEPT_ID IN (31, 42, 50); 
 --CAU10
 SELECT FIRST_NAME ||' '||LAST_NAME 
AS EMPLOYEE_NAME, START_DATE
FROM S_EMP 
WHERE EXTRACT (YEAR FROM START_DATE) = 1991; 
--CAU11 
 SELECT FIRST_NAME ||' '||LAST_NAME
AS EMPLOYEE_NAME, SALARY 
fROM S_EMP 
WHERE COMMISSION_PCT = 15; 
--CAU12 
SELECT LAST_NAME, 
       START_DATE,
       TO_CHAR(NEXT_DAY(ADD_MONTHS(START_DATE, 6), 'MONDAY'), 'Day "of" MONTH, YYYY') AS NEXT_MONDAY_DATE
FROM S_EMP;
--CAU13
SELECT  NAME
AS TENSP 
...
FROM  S_ITEM  ITE JOIN S_PRODUCT PRO
ON ITE.PRODUCT_ID = PRO.ID
WHERE ITE.ORD_ID=101; 
--CAU18 
SELECT CUS.ID, CUS.NAME, ORD.ID 
FROM S_CUSTOMER CUS LEFT JOIN S_ORD ORD 
ON CUS.ID = ORD.CUSTOMER_ID 
ORDER BY CUS.ID; 
--CAU19 
SELECT ORD.CUSTOMER_ID, ITE. PRODUCT_ID, ITE.QUANTITY
FROM S_ITEM ITE JOIN S_ORD ORD ON ITE. 
ORD_ID = ORD.ID 
...
BEGIN
    INSERT INTO s_customer (
        ...
        s_customer_id.NEXTVAL, p_name, p_phone, p_address, p_city, p_state,
        p_country, p_zip_code, p_credit_rating, p_sales_rep_id, p_region_id, p_comments
...
BEGIN
    v_new_customer_id := insert_customer('John Doe', '123-456-7890', '123 Main St', 'Anytown', 'CA', 'USA', '12345', 'GOOD', 101, 1, 'New customer');
...
BEGIN
    v_updated_rows := update_customer(1001, '123-456-7890', '456 Elm St', 'Othertown', 'CA', 'USA', '54321', 'EXCELLENT', 102, 2, 'Updated address');
    DBMS_OUTPUT.PUT_LINE('Updated Rows: ' || v_updated_rows);
END;
/
----
...
BEGIN
    ...
    -- Perform any additional salary calculations if needed
...
BEGIN
    v_employee_salary := calculate_employee_salary(201);
...
--
--
--LIMITING MAX STOCK LEVEL
CREATE OR REPLACE TRIGGER max_stock_trigger
BEFORE INSERT OR UPDATE ON s_inventory
FOR EACH ROW
DECLARE
    v_max_stock NUMBER;
BEGIN
    -- Retrieve the maximum stock level for the product
    SELECT max_in_stock INTO v_max_stock
    FROM s_inventory
    WHERE product_id = :NEW.product_id
    AND warehouse_id = :NEW.warehouse_id;

    IF :NEW.amount_in_stock > v_max_stock THEN
        RAISE_APPLICATION_ERROR(-20001, 'Stock level exceeds maximum allowed');
    END IF;
END;
/
--
...
--
------UPDATE -> NEW UPDATE
CREATE OR REPLACE TRIGGER emp_last_updated_trigger
...
BEFORE INSERT ON s_item
FOR EACH ROW
DECLARE
    v_item_total NUMBER;
BEGIN
    -- Calculate the total price for the item
    v_item_total := :NEW.price * :NEW.quantity;

    -- Update the total price for the item in the order
    :NEW.quantity_shipped := v_item_total;
END;
/
